...
namespace iHentai.Mvvm
{
    public abstract class ViewModel : INotifyPropertyChanged
    {
        static ViewModel()
        {
            KnownViews = ((Application.Current as IMvvmApplication)?.MvvmViewAssemblies()?.SelectMany(item => item.DefinedTypes) ?? Application.Current.GetType().GetTypeInfo().Assembly.DefinedTypes)
                .Select(item =>
                    item.IsClass &&
