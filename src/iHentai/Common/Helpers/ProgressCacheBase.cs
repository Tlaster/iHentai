...
namespace iHentai.Common.Helpers
{
    internal abstract class ProgressCacheBase<T>
    {
        ...
        {
            return GetItemAsync(uri, throwOnError, !storeToMemoryCache, cancellationToken, progress, null);
        }
        ...
        {
            return GetItemAsync(uri, throwOnError, false, cancellationToken, progress, initializerKeyValues);
        }
                    ...
                    while (retries < RetryCount)
                    {
                        try
                        {
                            instance = await DownloadFileAsync(uri, baseFile, preCacheOnly, cancellationToken,
                                progress, initializerKeyValues).ConfigureAwait(false);
